...
class CSV_EDITOR:
	...
	def menuBar(self):
		...
		self.file_menu.add_command(label="Edit", command=self.file_edit, accelerator="Ctrl+E")
	...
	def _build_tree(self):
		...
		style.configure(".", rowheight=40, highlightthickness=3, bd=2, font=('Ariel', 16), fieldbackground="white", fieldforeground="aqua", background= "LightGreen", foreground="blue", height= 4, wodth=4, ipady=4, ipadx=4)
	...
	def _widgets(self):
		...
		self.var = StringVar()
		label = Label(self.app, bg='LightGreen', bd=2, font=('Ariel', 16), justify='left',
		text= " To edit a row, select it and past its conents from clipboard into Entry filed below\nThen Click Edit button to modify that row, finally save tree into a file")
		label.grid(column=0, row=1, rowspan=1, padx=10, sticky='we', in_=self.app)
		...
		entry.grid(column=0, row=2, rowspan=1, padx=10, sticky='we', in_=self.app)
		add_item.grid(column=0, row=3, rowspan=1, sticky='we', in_=self.app)
		edit.grid(column=0, row=4, rowspan=1, sticky='we', in_=self.app)
		save.grid(column=0, row=5, rowspan=1, sticky='we', in_=self.app)
		delrec.grid(column=0, row=6, rowspan=1, sticky='we', in_=self.app)
		select.grid(column=0, row=7, rowspan=1, sticky='we', in_=self.app)
	...
	def save(self):
		...
		with open("{}".format(self.file), "w") as f:
			...
			for item in table:
				...
				f.writelines(",".join(["{}".format(x) for x in item]).strip()+"\n")
		messagebox.showinfo("File Saved", "{} saved succesfully".format(self.file))
