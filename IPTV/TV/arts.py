...
titles = []
stations = []
text = open("C:\\PY\\TV\\Playlists\\arts.txt", "r").read()
lines = text.split("\n")
for line in lines:
	if "EXTINF" in line:
...
def play_video(station):
	#os.system("c:/ffmpeg/bin/ffplay -fs  -vcodec  h264  -x  1920 -y  1080 -f hls  -infbuf  -exitonmousedown -nostats -loglevel 0 {}".format(station))
